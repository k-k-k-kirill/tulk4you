...
abstract class AbstractPostType {
	...
	public function set_labels() {
		...
		$labels = array(
			'name'               => sprintf(
				_x( '%s', 'post type general name', 'tulk4you-plugin' ),
				$this->post_label_plural
			),
			'singular_name'      => sprintf( _x( '%s', 'post type singular name', 'tulk4you-plugin' ), $this->post_label_singular ),
			'menu_name'          => sprintf( _x( '%s', 'admin menu', 'tulk4you-plugin' ), $this->post_label_plural ),
			'name_admin_bar'     => sprintf( _x( '%s', 'add new on admin bar', 'tulk4you-plugin' ), $this->post_label_singular ),
			'add_new'            => sprintf( _x( 'Add New', 'add new item', 'tulk4you-plugin' ), $this->post_label_singular ),
			'add_new_item'       => sprintf( __( 'Add New %s', 'tulk4you-plugin' ), $this->post_label_singular ),
			'new_item'           => sprintf( __( 'New %s', 'tulk4you-plugin' ), $this->post_label_singular ),
			'edit_item'          => sprintf( __( 'Edit %s', 'tulk4you-plugin' ), $this->post_label_singular ),
			'view_item'          => sprintf( __( 'View %s', 'tulk4you-plugin' ), $this->post_label_plural ),
			'all_items'          => sprintf( __( 'All %s', 'tulk4you-plugin' ), $this->post_label_plural ),
			'search_items'       => sprintf( __( 'Search %s', 'tulk4you-plugin' ), $this->post_label_plural ),
			'parent_item_colon'  => sprintf( __( 'Parent %s:', 'tulk4you-plugin' ), $this->post_label_plural ),
			'not_found'          => sprintf( __( 'No %s found.', 'tulk4you-plugin' ), $this->post_label_plural ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash.', 'tulk4you-plugin' ), $this->post_label_plural ),
		);
