...
class Compatibility {
	...
	public static function check_php_version() : bool {
		...
		if ( version_compare( self::PHP_VERSION, phpversion(), '>=' ) ) {
			// phpcs:ignore
			wp_die( esc_attr( __( 'You must be using PHP ' . self::PHP_VERSION . ' or greater.', 'tulk4you-theme' ) ), esc_attr( __( 'Theme &rsaquo; Error', 'tulk4you-theme' ) ) );
	...
	public static function check_wordpress_version() : bool {
		...
		if ( self::wp_version_applies( self::WP_VERSION, get_bloginfo( 'version' ) ) ) {
			...
			// phpcs:ignore
			wp_die( esc_attr( __( 'You must be using WordPress ' . self::WP_VERSION . ' or greater.', 'tulk4you-theme' ) ), esc_attr( __( 'Theme &rsaquo; Error', 'tulk4you-theme' ) ) );
